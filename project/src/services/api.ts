...
async login(email: string, password: string): Promise<ApiResponse<{ user: User; token: string; session: any }>> {
  try {
    ...
    if (error || !userData) {
      return createResponse(false, undefined, 'Invalid email or password');
    }

    // Check active user
    if (!userData.is_active) {
      return createResponse(false, undefined, 'User account is inactive');
    }
    ...
    return createResponse(false, undefined, 'Login failed');
  }
...
async changePassword(
  ...
  try {
    ...
    if (!isValid) {
      return createResponse(false, undefined, 'Current password is incorrect');
    }
    ...
    if (updateError) {
      return createResponse(false, undefined, updateError.message);
    }
