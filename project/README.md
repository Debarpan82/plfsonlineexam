...
```bash
   ...
   VITE\_SUPABASE\_SERVICE\_ROLE\_KEY=your\_supabase\_service\_role\_key
   ```
...
* `survey\_sections` - Survey sections
* `questions` - Question bank
* `question\_options` - Answer options
* `test\_sessions` - Active test sessions
* `test\_answers` - User responses
* `test\_results` - Final results
* `section\_scores` - Section-wise performance
* `certificates` - Generated certificates
* `system\_settings` - Configuration
* `activity\_logs` - Audit trail
...
│ Publishable │ sb\_publishable\_ACJWlzQHlZjBrEguHvfOxg\_3BJgxAaH │

│ Secret      │ sb\_secret\_N7UND0UgjKTVK-Uodkm0Hg\_xSvEMPvz      │
